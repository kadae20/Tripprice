...
function search(hotelIds, opts = {}) {
  ...
  const body = JSON.stringify({
    criteria: {
      ...
      // 실제 필드명: checkInDate / checkOutDate / hotelId (array)
      checkInDate:  opts.checkIn  || fmt(tomorrow),
      checkOutDate: opts.checkOut || fmt(dayAfter),
    ...
    const req = http.request({
      ...
      res.on('end', () => {
        try {
          ...
          // error 객체 처리 (HTTP 200이지만 데이터 없음: id=911 No search result 등)
          if (parsed.error) {
            const msg = parsed.error.message || `id=${parsed.error.id}`;
            console.log(`  ℹ  Affiliate Lite: ${msg}`);
            return resolve([]);
          }
