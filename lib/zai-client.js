...
async function generateHotelDraft(brief) {
  ...
  const hotelList = hotels.map(h => {
    ...
    const agodaId  = h.agoda_hotel_id || h.agodaHotelId || '';
    // partner_url (CID 포함) 우선 — 없으면 agodaId로 구성 — 최후 제네릭
    const ctaUrl   = h.partner_url || h.landing_url ||
      (agodaId
    ...
    // 추가 메타 (프롬프트 품질 향상)
    const extras = [];
    if (h.star_rating)  extras.push(`${h.star_rating}성급`);
    if (h.chain_name)   extras.push(`체인:${h.chain_name}`);
    if (h.yearopened)   extras.push(`개관:${h.yearopened}년`);
    if (h.numberrooms)  extras.push(`객실수:${h.numberrooms}개`);
    if (h.checkin_time) extras.push(`체크인:${h.checkin_time}`);
    if (h.rating_average && parseFloat(h.rating_average) > 0)
      extras.push(`평점:${h.rating_average}(${h.number_of_reviews || 0}건)`);

    // 영문 overview → 한국어 글 작성에 활용할 수 있게 전달
    const overview = (h.location_description || h.overview || '').slice(0, 150).trim();

    const line = `- **${name}**: 장점(${pros}), 단점(${cons}), 위치(${location}), 가격대(${price}), CTA_URL(${ctaUrl})`;
    return overview
      ? line + `\n  > 소개: ${overview}${extras.length ? ' | ' + extras.join(', ') : ''}`
      : (extras.length ? line + `\n  > 기본정보: ${extras.join(', ')}` : line);
  }).join('\n');
  ...
  const systemPrompt = [
    ...
    '광고·홍보 문체 금지. 장점과 단점 모두 서술. 사실·데이터 기반.',
    '개인 경험 단정 표현 금지 ("직접 투숙해봤는데", "제가 묵었을 때" 등).',
    '대신 "투숙객 후기 기준", "공식 자료 기준", "(아고다 리뷰 기준)" 등 근거를 명시.',
    '호텔 목록에 "소개:" 또는 "기본정보:"가 있으면 반드시 해당 정보를 본문에 자연스럽게 녹여 쓸 것.',
    '성급·체인·개관연도·객실수·체크인 시간은 구체적 수치로 언급 (예: "5성급 체인 호텔", "2010년 개관", "체크인 15:00").',
  ...
  const userPrompt = [
    ...
    `  5. 자주 묻는 질문 (FAQ 3개 이상 필수, **Q:** / **A:** 형식)`,
    `- 각 호텔 섹션 끝에 추천 요약 3줄 블록 필수:`,
    `  > **추천:** [이런 여행자에게 맞음 — 1줄]`,
    `  > **비추천:** [이런 경우 맞지 않음 — 1줄]`,
    `  > **주의:** [예약·투숙 시 주의사항 — 1줄]`,
    `- 각 호텔 섹션 끝에 CTA 삽입: "[호텔명 현재 가격 확인하기 →](CTA_URL)" — CTA_URL은 호텔 목록에서 그대로 사용`,
    ...
    `- 내부링크 2개 이상 자연스럽게 본문에 삽입: [관련 글 제목](/ko/related-slug) 형식`,
    `  예: "서울 지역별 호텔 동선 가이드는 [서울 호텔 동네별 추천](/ko/seoul-hotel-by-area)을 참고하세요."`,
    `- meta_description: 120~155자로 글 맨 끝에 별도 줄로 출력 (형식: META_DESC: {내용})`,
