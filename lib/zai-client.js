...
async function generateHotelDraft(brief) {
  ...
  // ── 실제 발행된 포스트 목록 로드 (내부링크 전용) ─────────────────────────
  let publishedPostsList = '(발행된 글 없음 — 내부링크 삽입 생략)';
  try {
  ...
  const userPrompt = [
    ...
    `- 내부링크: 아래 "실제 발행된 포스트 목록"에서만 골라 2개 이상 자연스럽게 삽입. 목록에 없는 URL 절대 사용 금지.`,
    `  형식: [글 제목](URL)`,
    ...
    ``,
    `## 실제 발행된 포스트 목록 (내부링크는 이 목록에서만 선택)`,
    publishedPostsList,
  ].filter(l => l !== '').join('\n');
