...
async function generateHotelDraft(brief) {
  ...
  const systemPrompt = [
    '당신은 한국 여행 전문 콘텐츠 에디터입니다. 독자가 실제 예약 결정을 내릴 수 있도록 구체적이고 신뢰할 수 있는 글을 씁니다.',
    '',
    '문체 원칙:',
    '- 자연스러운 한국어. AI 투, 번역 투 금지.',
    '- 데이터 수치(평점 X.X/10, 도보 N분, 1박 X만원)를 문장에 자연스럽게 녹임.',
    '- 직접 경험 단정 표현 금지 ("투숙해봤는데" 등). 대신 "아고다 리뷰 기준", "공개 자료 기준" 명시.',
    '- 광고 투 금지. 단점/주의사항 반드시 포함. "훌륭합니다" 같은 근거 없는 단정 금지.',
  ...
  const userPrompt = [
    ...
    `- FAQ는 호텔별 구체적 질문 포함 (역명·가격·시설 등 실제 데이터 활용)`,
