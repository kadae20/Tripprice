...
// ── 검증 G: 멀티라인 CSV + NUL sanitize + raw 도시명 매칭 ───────────────────
console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
console.log(' [G] 멀티라인 CSV + NUL sanitize + raw 도시명(한국어) 매칭 검증');
console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

// 멀티라인 + NUL + 한국어 도시명이 섞인 CSV
// overview 필드에 embedded newline이 있는 quoted 필드
const MULTILINE_CSV_CONTENT =
  'hotel_id,hotel_name,city,overview,agoda_hotel_id,content_priority\n' +
  'multi-seoul,\uC11C\uC6B8\uD638\uD154,\uC11C\uC6B8,"서울 도심에 위치.\n조망이 훌륭합니다.",200001,normal\n' +
  'nul-busan,\uBD80\uC0B0\uD638\uD154,\u0000\uBD80\uC0B0\u0000,"부산 해운대",200002,normal\n' +
  'skip-tokyo,\uB3C4\uAFFC\uD638\uD154,\uB3C4\uAFFC,"\uB3C4\uAFFC \uC2E0\uC8FC\uCFE0",200003,normal\n';

const tmpDir3 = fs.mkdtempSync(path.join(require('os').tmpdir(), 'tripprice-multi-'));
const tmpMultiLatest = path.join(tmpDir3, 'hotels-multi-latest.csv');
const tmpMultiSubset = path.join(tmpDir3, 'hotels-multi-subset.csv');
fs.writeFileSync(tmpMultiLatest, MULTILINE_CSV_CONTENT, 'utf8');

let multiRunErr = false;
try {
  execFileSync(process.execPath, [path.join(SCRIPTS, 'hoteldata-extract.js')], {
    env: {
      ...process.env,
      HOTELDATA_LATEST_CSV:     tmpMultiLatest,
      HOTELDATA_SUBSET_CSV:     tmpMultiSubset,
  ...
  multiRunErr = true;
  console.error('  멀티라인 테스트 extract 실패:', (err.stderr || err.message).slice(0, 300));
}

test('멀티라인 CSV: overview에 embedded newline 있어도 추출됨 (서울 호텔)', () => {
  assert(!multiRunErr, 'extract가 실패함 (멀티라인 CSV)');
  assert(fs.existsSync(tmpMultiSubset), 'subset 파일 미생성');
  const content = fs.readFileSync(tmpMultiSubset, 'utf8');
  assert(content.includes('multi-seoul'), '멀티라인 embedded newline 서울 호텔이 미포함');
});

test('NUL 포함 city 필드 sanitize 후 부산 호텔 포함됨', () => {
  assert(!multiRunErr && fs.existsSync(tmpMultiSubset), 'subset 파일 없음');
  const content = fs.readFileSync(tmpMultiSubset, 'utf8');
  assert(content.includes('nul-busan'), 'NUL 제거 후 부산 호텔이 미포함');
  assert(!content.includes('skip-tokyo'), '도쿄 호텔이 포함됨 (타겟 외 도시)');
});

// 단위 테스트: raw 한국어 city 우선 매칭
test('raw 한국어 city 우선 매칭 (서울 raw → cityMatches 성공)', () => {
  const targetsRaw2   = new Set(['서울']);
  const targetCities2 = buildTargetCities(['서울']);
  assert(cityMatches('서울', targetCities2, targetsRaw2), '서울 raw 매칭 실패');
  assert(!cityMatches('도쿄', targetCities2, targetsRaw2), '도쿄가 서울 타겟에 매칭됨');
});
